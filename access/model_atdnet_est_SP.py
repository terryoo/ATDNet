...
class denoiser(object):
    ...
    def __init__(self, sess, input_c_dim = 1):
        ...
        self.h_sigma, self.sigma_mask = Estimation_block2(self.X)
        image_shape = tf.unstack(tf.shape(self.X))
        self.sigma_matrix = tf.round(
            tf.image.resize_images(self.sigma_mask, [image_shape[1], image_shape[2]]) * 255) / 255.
